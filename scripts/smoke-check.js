...
(async function run() {
  ...
  console.log(`Smoke check: ${symbol} ${interval} from ${start.toISOString()} to ${end.toISOString()}`);
  try {
    const candles = await fetchCandles(symbol, interval, start.toISOString(), end.toISOString());
    if (!Array.isArray(candles)) throw new Error("fetchCandles returned non-array");
    ...
    if (!candles.every(c => c && c.time && typeof c.open === "number" && typeof c.high === "number" && typeof c.low === "number" && typeof c.close === "number")) {
    ...
    try {
      const alert = await alertForSymbol(symbol, { interval, period1: start.toISOString(), period2: end.toISOString() });
      console.log("alert result:", alert);
    } catch (ae) {
      console.error("alert check failed:", ae && ae.stack ? ae.stack : ae.message);
    }

    console.log("Smoke check succeeded");
    process.exit(0);
  } catch (err) {
    console.error("Smoke check failed:", err && err.stack ? err.stack : err.message);
    process.exit(2);
  }
})();
